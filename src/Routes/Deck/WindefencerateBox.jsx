...
const WindefenceRate = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await getDeckWindefenceRate();
        setInfo(result);
      } catch (e) {
  ...
  return (
    <body>
      <h2 className="mb-10 text-6xl text-white">순방률</h2>
      {info["data"]?.map((match) => (
        <DeckBody>
          <FirstBox>
            <BoxMain>
...
const BoxBodyImage = styled.div`
  /* background: url("https://cdn.dak.gg/tft/images2/tft/meta/champions/TFT8_Annie.jpg"); */
